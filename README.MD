...
The [Time Series Classification (CNN-SVM) Algorithm from AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-vzxmyw25oqtx6) 
...
A residual connection is applied between the input and the output of each block, where the input is transformed by a convolutional layer with a kernel size of 1 when its length does not match the one of the output.
...
and as distant as possible from the extracted features of other random subsequences (negative samples).
